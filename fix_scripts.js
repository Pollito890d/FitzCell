...
function injectScript(file, newScript) {
    let content = fs.readFileSync(file, 'utf8');
    let idx = content.indexOf('<script>');
    if (idx !== -1) {
        content = content.substring(0, idx) + newScript;
        fs.writeFileSync(file, content, 'utf8');
    }
}

const sVentas = <script>
    ...
    function renderPOS(filter = '') {
      ...
      data.forEach(p => {
         ...
         <div class="bg-gray-950 border border-gray-800 rounded-xl p-4 flex justify-between items-center hover:border-gray-700 transition cursor-pointer">
           <div><p class="font-bold">\</p><p class="text-sm text-gray-500">Stock: \</p></div>
           <div class="flex items-center gap-4">
             <span class="font-bold text-brand">\</span>
             <button onclick="addTicket(\, '\', \)" class="bg-gray-800 hover:bg-gray-700 px-3 py-1.5 rounded-lg text-white font-bold transition">Añadir</button>
           </div>
         </div>\;
      });
      ...
      ticketItems.forEach((i, idx) => {
        ...
        <div class="flex justify-between items-center text-sm">
          <div class="flex items-center gap-2 flex-1">
            <span class="bg-gray-800 px-2 rounded font-bold text-xs cursor-pointer text-red-400" onclick="removeTicket(\)">x</span>
            <span class="bg-gray-800 px-2 rounded font-bold text-xs">x\</span>
    ...
    function renderCatalogo() {
      ...
      inventarioDB.forEach(p => {
        ...
        <tr class="hover:bg-gray-800/50 transition">
          ...
          <td class="p-4 font-bold text-brand">\</td>
          <td class="p-4 text-center"><span class="\">\</span></td>
        </tr>\;
      });
          ...
          data.forEach(rep => {
             ...
             <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:border-brand/50 transition flex flex-col">
              <div class="flex justify-between items-start mb-4">
                ...
                <span class="px-3 py-1 text-xs font-bold rounded-full \">\</span>
              </div>
              <div class="mb-6 flex-1">
                <p class="text-sm text-gray-300 font-bold mb-1">Dispositivo ID: \</p>
                <p class="text-sm text-gray-500 italic">"\"</p>
              </div>
              <button onclick="irADiagnostico('\', '\', '\', '\')" class="w-full bg-gray-800 hover:bg-gray-700 text-white py-2 rounded-lg text-sm font-bold transition">Atender Equipo</button>
            </div>\;
          });
          ...
          if(matches.length > 0) {
            ...
            matches.forEach(m => {
              ...
              <div class="p-3 border-b border-gray-800 hover:bg-gray-800 cursor-pointer flex justify-between" 
                ...
                <span class="text-xs text-brand bg-brand/10 px-2 rounded">Stock: \</span>
              </div>\;
            });
    ...
    function agregarPieza(nombre, id) {
       ...
       div.innerHTML = \
          <div>
            ...
            <p class="text-xs text-gray-500">ID Producto: \</p>
          </div>
...
const sEncargado = <script>
          ...
          data.slice(0, 3).forEach(rep => {
            html += \
             <div class="flex justify-between items-center bg-gray-950 p-4 rounded-xl border border-gray-800">
               <div><p class="font-bold">\</p></div>
               <span class="px-3 py-1 bg-brand/10 text-brand text-xs font-bold rounded-full border border-brand/20">\</span>
             </div>\;
          });
    ...
    function renderCategorias() {
      ...
      cats.forEach(c => {
        if(c) {
          html += \<button onclick="renderInventario('\')" class="whitespace-nowrap px-4 py-2 bg-gray-800 hover:bg-brand text-white rounded-lg text-sm font-bold transition">\</button>\;
        }
      });
        ...
        if(res.ok) {
          ...
          data.forEach(rep => {
            ...
            <tr class="hover:bg-gray-800/50 transition">
              ...
              <td class="p-4 text-gray-400">Equipo ID: \</td>
              <td class="p-4 text-gray-400 truncate max-w-xs">\</td>
              <td class="p-4 font-bold text-brand">\</td>
              <td class="p-4 text-center"><span class="px-2 py-1 bg-gray-800 rounded text-xs border border-gray-700">\</span></td>
            </tr>\;
          });
...
injectScript('./admin/dashboard_ventas.html', sVentas);
injectScript('./admin/dashboard_tecnico.html', sTecnico);
injectScript('./admin/dashboard_encargado.html', sEncargado);
